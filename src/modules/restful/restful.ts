...
export class Restful extends BaseRestful {
    ...
    protected getRouteDocs(
        ...
        const mergeDoc = (vDoc: Omit<SwaggerOption, 'include'>, route: RouteOption) => ({
            ...
            path: genDocPath(route.path, this.config.docurl, parent),
    ...
    protected getDocOption(name: string, voption: VersionOption, isDefault = false) {
        ...
        const defaultDoc = {
            ...
            version: name,
            path: trim(`${this.config.docurl}${isDefault ? '' : `/${name}`}`, '/'),
        };
        // 获取路由文档
        ...
        if (include.length > 0 || !docConfig.routes) docConfig.default = { ...defaultDoc, include };
        return docConfig;
    }
    ...
    async factoryDocs<T extends INestApplication>(
        ...
        for (const voption of docs) {
            const { title, description, auth, tags, version, include } = voption;
