...
export class UniqueTreeConstraint implements ValidatorConstraintInterface {
    ...
    async validate(value: any, args?: ValidationArguments) {
        ...
        try {
            ...
            // 对比每个子分类的queryProperty值是否与当前验证的dto的属性相同，如果有相同的则验证失败
            return collection.every((item) => item[condition.parentKey] !== value);
        } catch (err) {
            return false;
        }
    }
    ...
    defaultMessage(args?: ValidationArguments) {
        ...
        const queryProperty = property ?? args.constraints[0];
        if (!entity) {
